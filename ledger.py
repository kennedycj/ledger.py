...
def parse_amount(amount_string):
    ...
    mydict = {36: None, 44: None}
    quantity = int(round(float(amount_string.translate(mydict)) * 100.0))
...
def parse_amount_adjusting_sign(account_string, amount_string):
    ...
    quantity = int(round(float(amount_string.translate(mytable)) * 100.0))
...
def extract_single_unit_quantity(amounts):
    ...
    if amounts_list:
        return amounts_list[0]['quantity']
...
def print_transactions(transactions):
    ...
    for transaction in transactions:
        ...
        for posting in transaction['postings']:
            print(' {} {} '.format(posting['account'], format_amount(posting['amount'])))
        print
...
def parse_balance_verify(line_number, line, adjust_sign):
    ...
    mydict = {36: None}
    amount_string = split[3].translate(mydict)
...
def parse_posting(line_number, line, adjust_sign):
    ...
    amount_string = split[1].translate(str.maketrans('', '', '$'))
...
def _ensure_sub_account(account_node,
    ...
    if not sub_account_regular_name in account_node.sub_accounts:
